...
seple = function(track,AvailList,locAvailFile,UseDist, start.val = NULL)
{
require(MASS)
...
#Return a list of results
    list(parTable=(parTable),
	    ...
	    AICc = AICc,
          ...
          convergence=(mle.sep$convergence==0), UseDist)
}
...
sbvnle = function(track,AvailList,locAvailFile,UseDist,start.val = NULL)
{
require(MASS)
habmat = AvailList[[1]] #Just take the first one for now...
...
#Return a list of results
    list(parTable=(parTable),
          ...
          AICc = AICc,
          evalTime = difftime(etime,stime),
          convergence=(mle.sbvn$convergence==0), UseDist)
}
...
SumLogLik = 0
Log.Loc.g.u=array(0,nrow(track))
...
density = array(Loc.g.a*wLoc/MapVolume)
if(is.na(density)){density=10^-320}
if(density==0){density=10^-320}
LogLoc.g.u[i]=log(density)
SumLogLik = SumLogLik+LogLoc.g.u[i]
